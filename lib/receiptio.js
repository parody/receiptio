    ...
    return new Promise(async resolve => {
        ...
        const start = () => {
            ...
            switch (printer.command) {
                ...
                case 'hwasungusb':
                    mode = 'escpos';              
                    hello = '\x10\xaaU\x80T\xab'; // DLE+AAh+U+80h+T+ABh - HWASUNG USB uses this instead of DLE EOT
                    break;
                default:
                    break;
            }
...
const convertOption = params => {
    ...
    if (!/^(svg|png|text|escpos|epson|sii|citizen|fit|impactb?|generic|star(line|graphic|impact[23]?)?|emustarline|hwasungusb)$/.test(p)) {
...
const rasterize = async (receiptmd, printer, encoding) => {
    ...
    if (puppeteer) {
        ...
        // show a warn when trying to rasterize a ticket without sharp/puppeteer
        console.warn("receiptio: Unable to rasterize");
        return '';
    }
};
