...
const parseOption = options => {
    ...
    const params = {
        ...
        R: false, // RAW input (do not transform input when printing)
...
const convertOption = params => {
    ...
    // adjust cpl according to margin
    const finalMargin = m >= 0 && m <= 96 ? Math.trunc(m) : 0;
    const finalCpl = (c >= 24 && c <= 96 ? Math.trunc(c) : 48) - finalMargin;
...
const transform = async (receiptmd, printer) => {
    ...
    if (printer.leftMargin) {
        printer.command = typeof printer.command === 'object' ? printer.command : receiptline.commands[printer.command];
